...
import v1.article.ArticleResource
...
class ArticleRouterSpec extends PlaySpec with GuiceOneAppPerTest {
  ...
  "PostRouter" should {
    ...
    "render the list of posts" in {
      ...
      val posts: Seq[ArticleResource] = Json.fromJson[Seq[ArticleResource]](contentAsJson(home)).get
      posts.filter(_.id == "1").head mustBe (ArticleResource("1","/v1/posts/1", "title 1", "blog post 1" ))
    }
